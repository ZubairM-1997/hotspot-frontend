import React from 'react'
import WeatherCard from './WeatherCard'





//Optional include of the default css styles


export default class Weather extends React.Component {

	state = {
		weather: ""
	}



	componentDidUpdate(prevProps, prevState){
		if (this.props.weather.lat && prevProps.weather.lat !== this.props.weather.lat) {
			fetch(`http://api.worldweatheronline.com/premium/v1/weather.ashx?key=acbec1d6ae0e477a95000157193009&q=${this.props.weather.lat},${this.props.weather.long}&format=json`)
			.then(resp => resp.json())
			.then((data) => this.temp({...data}))

		}
	}

	temp = (data) => {
		this.setState({
			weather: data.data.current_condition[0]
		})

	}

	render(){
		return(
			<div className="card">
				<h2>Weather</h2>
				<WeatherCard currentWeather={this.state.weather}/>
			</div>
		)
	}
}

